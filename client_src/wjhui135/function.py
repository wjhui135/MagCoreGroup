...
def Attack(gameID, playerID, x, y):
    url = "http://test.magcore.clawit.com/api/cell/"
    payload = "{\"Game\":\"%s\",\"Player\":\"%s\", \"X\":%d, \"Y\":%d}" % (gameID, playerID, x, y)
    headers = {
        'Content-Type': "application/json",
        'Cache-Control': "no-cache"
    }
...
#     gameID = "62043a76d008416a9593d7ad383fe8a8"
...
#     game = GetGame(gameID)
#     MapName = input("请输入地图名称（rectsmall、rectMid、RectLarge）：")
#     Map = GetMap(MapName)
